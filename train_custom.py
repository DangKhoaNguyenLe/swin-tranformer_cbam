...
NUM_EPOCHS = 50
LEARNING_RATE = 5e-4      # Cho CBAM module
HEAD_LR = 5e-4            # Cho Head và Bottleneck
BACKBONE_LR = 5e-5         # Cho Swin Backbone
PATIENCE = 10        

def main(dataset_path):
    print("=" * 60)
    print(" BẮT ĐẦU HUẤN LUYỆN SWIN-CBAM VỚI RMFRD (ULTIMATE CONFIG)")
    print("=" * 60)
    ...
    # Import SwinWithCBAM
    swin_cbam_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Sw-cbam", "swin_cbam.py")
    spec = importlib.util.spec_from_file_location("swin_cbam", swin_cbam_path)
    swin_cbam_module = importlib.util.module_from_spec(spec)
    ...
    # KHÔNG ĐÓNG BĂNG BACKBONE (Huấn luyện ngang hàng với Swin thuần từ đầu)
    ...
    optimizer = optim.AdamW([
        ...
        {'params': cbam_params, 'lr': LEARNING_RATE},
        {'params': alpha_params, 'lr': LEARNING_RATE * 2, 'weight_decay': 0.0}, # Bỏ weight decay để alpha không bị ép về 0   
    ...
    if os.path.exists(custom_weights_path):
        ...
        backbone_module = model.module.feature_extractor.backbone if isinstance(model, nn.DataParallel) else model.feature_extractor.backbone
